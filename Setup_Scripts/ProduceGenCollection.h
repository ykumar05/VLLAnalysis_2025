...
{
  int grandmomid=-1;
  unsigned int iterator_gP = (unsigned int)*nGenPart;
  for(unsigned int i=0; i<iterator_gP; i++){
    ...
    bool motherisnotmu= abs(MotherID(i,GenPart_genPartIdxMother[i]))!=13;
...
void AnaScript::GenElectron()
{
  ...
  for(unsigned int i=0; i<iterator_gP; i++){
    ...
    bool motherisnotele= abs(MotherID(i,GenPart_genPartIdxMother[i]))!=11;
...
void AnaScript::GenPhoton()
{
  ...
  for(unsigned int i=0; i<iterator_gP; i++){
    ...
    bool passcutPho     = abs(temp.pdgid)==22 && temp.v.Pt()>10 && fabs(temp.v.Eta())<2.4;
    bool motherisnotPho = abs(MotherID(i,GenPart_genPartIdxMother[i]))!=22;
    
    passcutPho = passcutPho && motherisnotPho;

    if(passcutPho)
      genPhoton.push_back(temp);
...
void AnaScript::createSignalArrays(){
  ...
  for(unsigned int i=0; i<iterator_gP; i++){
    ...
    unsigned int iterator_gP = (unsigned int)*nGenPart;
    for(unsigned int j=0; j<iterator_gP; j++){
